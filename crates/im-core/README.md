...
AuthZ is NOT enforced in identity-matters v1. This crate locks the shared IAM boundary for callers today, while the v2+ roadmap replaces `im-stub` with an enforcing `im-daemon` behind the same `Authorizer` contract.

`im-core` owns the stable authorization types:
