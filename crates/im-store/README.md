...
`im-store` keeps IAM audit data inside identity-matters. Consumers query it through `SqliteAuditSink` and `query_audit` rather than hosting identity data in their own stores.
